package tables
// AUTO-GENERATED Slick data model for table EventsStagesCurrent
trait EventsStagesCurrentTable {
  ...
  /** Entity class storing rows of table EventsStagesCurrent
  ...
  case class EventsStagesCurrentRow(THREAD_ID: Long, EVENT_ID: Long, END_EVENT_ID: Option[Long], EVENT_NAME: String, SOURCE: Option[String], TIMER_START: Option[Long], TIMER_END: Option[Long], TIMER_WAIT: Option[Long], NESTING_EVENT_ID: Option[Long], NESTING_EVENT_TYPE: Option[String])
  /** GetResult implicit for fetching EventsStagesCurrentRow objects using plain SQL queries */
  implicit def GetResultEventsStagesCurrentRow(implicit e0: GR[Long], e1: GR[Option[Long]], e2: GR[String], e3: GR[Option[String]]): GR[EventsStagesCurrentRow] = GR{
    prs => import prs._
    EventsStagesCurrentRow.tupled((<<[Long], <<[Long], <<?[Long], <<[String], <<?[String], <<?[Long], <<?[Long], <<?[Long], <<?[Long], <<?[String]))
  }
  /** Table description of table events_stages_current. Objects of this class serve as prototypes for rows in queries. */
  class EventsStagesCurrent(_tableTag: Tag) extends profile.api.Table[EventsStagesCurrentRow](_tableTag, Some("performance_schema"), "events_stages_current") {
    def * = (THREAD_ID, EVENT_ID, END_EVENT_ID, EVENT_NAME, SOURCE, TIMER_START, TIMER_END, TIMER_WAIT, NESTING_EVENT_ID, NESTING_EVENT_TYPE).<>(EventsStagesCurrentRow.tupled, EventsStagesCurrentRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(THREAD_ID), Rep.Some(EVENT_ID), END_EVENT_ID, Rep.Some(EVENT_NAME), SOURCE, TIMER_START, TIMER_END, TIMER_WAIT, NESTING_EVENT_ID, NESTING_EVENT_TYPE)).shaped.<>({r=>import r._; _1.map(_=> EventsStagesCurrentRow.tupled((_1.get, _2.get, _3, _4.get, _5, _6, _7, _8, _9, _10)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
  ...
  /** Collection-like TableQuery object for table EventsStagesCurrent */
  lazy val EventsStagesCurrent = new TableQuery(tag => new EventsStagesCurrent(tag))
}
