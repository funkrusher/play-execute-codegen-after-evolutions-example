...
trait ProductTable {
  ...
  self:TablesRoot with ClientTable  =>
   ...
   *  @param deleted Database column deleted SqlType(BOOLEAN), Default(false) */
  case class ProductRow(productId: Long, clientId: Int, price: scala.math.BigDecimal, createdAt: java.sql.Timestamp, updatedAt: java.sql.Timestamp, deleted: Boolean)
  ...
  class Product(_tableTag: Tag) extends profile.api.Table[ProductRow](_tableTag, Some("testshop"), "product") {
    ...
    /** Database column deleted SqlType(BOOLEAN), Default(false) */
