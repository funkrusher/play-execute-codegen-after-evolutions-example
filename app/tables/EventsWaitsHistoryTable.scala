package tables
// AUTO-GENERATED Slick data model for table EventsWaitsHistory
trait EventsWaitsHistoryTable {
  ...
  /** Entity class storing rows of table EventsWaitsHistory
  ...
  case class EventsWaitsHistoryRow(THREAD_ID: Long, EVENT_ID: Long, END_EVENT_ID: Option[Long], EVENT_NAME: String, SOURCE: Option[String], TIMER_START: Option[Long], TIMER_END: Option[Long], TIMER_WAIT: Option[Long], SPINS: Option[Long], OBJECT_SCHEMA: Option[String], OBJECT_NAME: Option[String], INDEX_NAME: Option[String], OBJECT_TYPE: Option[String], OBJECT_INSTANCE_BEGIN: Long, NESTING_EVENT_ID: Option[Long], NESTING_EVENT_TYPE: Option[String], OPERATION: String, NUMBER_OF_BYTES: Option[Long], FLAGS: Option[Long])
  /** GetResult implicit for fetching EventsWaitsHistoryRow objects using plain SQL queries */
  implicit def GetResultEventsWaitsHistoryRow(implicit e0: GR[Long], e1: GR[Option[Long]], e2: GR[String], e3: GR[Option[String]]): GR[EventsWaitsHistoryRow] = GR{
    prs => import prs._
    EventsWaitsHistoryRow.tupled((<<[Long], <<[Long], <<?[Long], <<[String], <<?[String], <<?[Long], <<?[Long], <<?[Long], <<?[Long], <<?[String], <<?[String], <<?[String], <<?[String], <<[Long], <<?[Long], <<?[String], <<[String], <<?[Long], <<?[Long]))
  }
  /** Table description of table events_waits_history. Objects of this class serve as prototypes for rows in queries. */
  class EventsWaitsHistory(_tableTag: Tag) extends profile.api.Table[EventsWaitsHistoryRow](_tableTag, Some("performance_schema"), "events_waits_history") {
    def * = (THREAD_ID, EVENT_ID, END_EVENT_ID, EVENT_NAME, SOURCE, TIMER_START, TIMER_END, TIMER_WAIT, SPINS, OBJECT_SCHEMA, OBJECT_NAME, INDEX_NAME, OBJECT_TYPE, OBJECT_INSTANCE_BEGIN, NESTING_EVENT_ID, NESTING_EVENT_TYPE, OPERATION, NUMBER_OF_BYTES, FLAGS).<>(EventsWaitsHistoryRow.tupled, EventsWaitsHistoryRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(THREAD_ID), Rep.Some(EVENT_ID), END_EVENT_ID, Rep.Some(EVENT_NAME), SOURCE, TIMER_START, TIMER_END, TIMER_WAIT, SPINS, OBJECT_SCHEMA, OBJECT_NAME, INDEX_NAME, OBJECT_TYPE, Rep.Some(OBJECT_INSTANCE_BEGIN), NESTING_EVENT_ID, NESTING_EVENT_TYPE, Rep.Some(OPERATION), NUMBER_OF_BYTES, FLAGS)).shaped.<>({r=>import r._; _1.map(_=> EventsWaitsHistoryRow.tupled((_1.get, _2.get, _3, _4.get, _5, _6, _7, _8, _9, _10, _11, _12, _13, _14.get, _15, _16, _17.get, _18, _19)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
  ...
  /** Collection-like TableQuery object for table EventsWaitsHistory */
  lazy val EventsWaitsHistory = new TableQuery(tag => new EventsWaitsHistory(tag))
}
